"""
OpenAI API plugin for the LLM Cybersecurity Benchmark system.

This module provides an OpenAI API-based plugin with comprehensive rate limiting,
...
class APIRateLimiter:
    """Rate limiter for API requests to prevent hitting OpenAI rate limits."""
...
class CostTracker:
    """Track API usage costs for different OpenAI models."""

    def __init__(self) -> None:
        """Initialize cost tracker with current OpenAI pricing."""
    ...
    def add_request(
        ...
        if not model_pricing:
            ...
            self.logger.warning(f"Unknown model {model}, using GPT-4o-mini pricing as fallback")
...
class OpenAIModelPlugin(ModelPlugin):
    """
    OpenAI API plugin with comprehensive rate limiting, cost tracking, and error handling.

    Features:
    - Support for all OpenAI GPT models (GPT-4o, GPT-4o-mini, GPT-4 Turbo, etc.)
    - Advanced rate limiting to prevent API violations
    - Accurate cost tracking with up-to-date pricing
    - Robust retry logic with exponential backoff
    - Cybersecurity-focused prompting and analysis
    ...
    def __init__(self) -> None:
        ...
        self.logger = get_logger("openai_plugin")

        # Initialize rate limiter and cost tracker
        self.rate_limiter = APIRateLimiter(requests_per_minute=60, tokens_per_minute=90000)
        self.cost_tracker = CostTracker()

        # Supported models (updated list)
        self.supported_models = {
            # GPT-4o models (latest and best)
            "gpt-4o": {"max_tokens": 4096, "context_window": 128000},
            "gpt-4o-2024-05-13": {"max_tokens": 4096, "context_window": 128000},
            "gpt-4o-2024-08-06": {"max_tokens": 4096, "context_window": 128000},
            ...
            "gpt-4-32k": {"max_tokens": 4096, "context_window": 32768},
            # GPT-3.5 Turbo
    ...
    async def initialize(self, config: dict[str, Any]) -> ServiceResponse:
        ...
        try:
            ...
            tokens_per_minute = config.get("tokens_per_minute", 90000)
            self.rate_limiter = APIRateLimiter(requests_per_minute, tokens_per_minute)

            self.logger.info(f"Initializing OpenAI API with model: {model_name}")
            ...
            self.model_info = ModelInfo(
                ...
                description=f"OpenAI {model_name} via API",
            ...
            self.logger.info(
                f"OpenAI model initialized successfully: {config['name']} ({model_name})"
            )

            return ServiceResponse(
                success=True,
                message=f"OpenAI model loaded: {config['name']}",
                data={
            ...
            return ServiceResponse(
                success=False,
                message=f"OpenAI model initialization failed: {str(e)}",
                error=f"OpenAI model initialization failed: {str(e)}",
            )
    ...
    async def _test_api_access(self) -> None:
        ...
        try:
            ...
            if response.choices and response.usage:
    ...
    async def predict(self, samples: list[str]) -> list[Prediction]:
        """
        Generate predictions using OpenAI API with cybersecurity analysis.
        ...
        """
        if self.async_client is None or self.model_name is None:
    ...
    def _format_cybersecurity_prompt(self, sample: str) -> str:
        ...
        """
        return f"""Analyze the following network log entry or security event for potential cybersecurity threats:
...
Analysis:"""
    ...
    def _parse_response(self, response: str) -> dict[str, Any]:
        """
        Parse structured response from OpenAI model.
    ...
    async def explain(self, sample: str) -> str:
        ...
        try:
            ...
            explanation_prompt = f"""As a cybersecurity expert, provide a comprehensive analysis of this security event:

Event: {sample}

Provide a detailed explanation covering:
1. What specific indicators you observed in this event
2. Why you classified it as attack/benign based on these indicators
3. Potential impact and severity if this is malicious
4. Recommended response actions and mitigation steps
5. Any similar attack patterns or techniques this might represent
...
Detailed Analysis:"""
            ...
            messages = [
                {
                    "role": "system",
                    "content": "You are a senior cybersecurity analyst providing detailed threat analysis and explanations.",
                },
                {"role": "user", "content": explanation_prompt},
            ]

            # Estimate tokens and make request
            estimated_tokens = len(explanation_prompt.split()) * 1.3 + 800
            response_data = await self._make_api_request(messages, int(estimated_tokens))
    ...
    async def health_check(self) -> dict[str, Any]:
        """
        Check the health status of the OpenAI API connection.
        ...
        try:
            status = {
                ...
                "client_connected": self.async_client is not None,
                "model_name": self.model_name,
            }

            # Test API connectivity
            if self.async_client and self.model_name:
                try:
                    ...
                    status["test_tokens_used"] = (
                        test_response.usage.total_tokens if test_response.usage else 0
                    )
    ...
    async def cleanup(self) -> None:
        ...
        try:
            self.logger.info("Cleaning up OpenAI API resources")
            ...
            self.logger.info("OpenAI API cleanup completed")
