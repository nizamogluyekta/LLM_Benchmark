#!/usr/bin/env python3
"""
NSL-KDD Network Intrusion Detection Benchmark with Google Gemini API

This script provides a specialized benchmarking solution for network intrusion detection
using the NSL-KDD dataset and Google Gemini API.

Requirements:
- Google AI API key (GOOGLE_API_KEY environment variable)
- NSL-KDD dataset in the NSL-KDD folder
- Python dependencies: google-generativeai, pandas, numpy, scikit-learn

Usage:
    export GOOGLE_API_KEY="your-api-key"
    python nsl_kdd_gemini_benchmark.py --quick-test
    python nsl_kdd_gemini_benchmark.py --full-benchmark --max-samples 1000
"""
...
class NSLKDDBenchmarkWithGemini:
    """Main benchmarking class for NSL-KDD intrusion detection with Gemini API."""
    ...
    async def run_benchmark(self, connections: list[NetworkConnection]) -> dict[str, Any]:
        ...
        logger.info(f"Starting benchmark with {len(connections)} connections using Gemini {self.model_name}")
        ...
        for i, connection in enumerate(connections):
            ...
            # Get prediction
            prediction_result = await self.gemini_client.predict_intrusion(connection)
            ...
            # Add delay to respect API rate limits (free tier: 50 requests/day)
            await asyncio.sleep(2.0)  # Increased delay for rate limiting
    ...
    def _calculate_metrics(self, predictions: list[dict], actual_labels: list[str], response_times: list[float], attack_types: dict, total_time: float) -> dict[str, Any]:
        ...
        y_true_binary = [1 if label == "ATTACK" else 0 for label in actual_labels]
        y_pred_binary = [1 if pred == "ATTACK" else 0 for pred in pred_labels]
        y_mean = np.mean(y_true_binary)
        ss_res = sum((y_true - y_pred) ** 2 for y_true, y_pred in zip(y_true_binary, y_pred_binary))
        ss_tot = sum((y_true - y_mean) ** 2 for y_true in y_true_binary)
    ...
    def print_results_summary(self, report: dict[str, Any]) -> None:
        ...
        print("\n" + "="*60)
        print("NSL-KDD GEMINI INTRUSION DETECTION BENCHMARK SUMMARY")
        print("="*60)
        ...
        print("="*50)
        am = metrics["advanced_metrics"]
        print(f"Matthews Correlation:    {am['matthews_correlation']:.3f}")
        print(f"R-Squared:               {am['r_squared']:.3f}")

        print("\n" + "="*50)
        print("ERROR RATES")
        print("="*50)
        er = metrics["error_rates"]
        print(f"False Positive Rate:     {er['false_positive_rate']:.3f}")
        print(f"False Negative Rate:     {er['false_negative_rate']:.3f}")
        ...
        if attack_performance:
            ...
            for attack_type, stats in attack_performance.items():
                ...
                print(f"{attack_type:<15}: {stats['correct']:>3}/{stats['total']:>3} ({accuracy:.3f})")
...
async def main():
    """Main function to run the NSL-KDD benchmark with Gemini."""

    parser = argparse.ArgumentParser(description="NSL-KDD Intrusion Detection Benchmark with Google Gemini API")
    parser.add_argument("--api-key", help="Google API key (or set GOOGLE_API_KEY env var)")
    parser.add_argument("--model-name", default="gemini-1.5-flash", help="Gemini model to use (default: gemini-1.5-flash)")
    parser.add_argument("--dataset-file", default="KDDTrain+.txt", help="NSL-KDD dataset file to use")
    parser.add_argument("--max-samples", type=int, default=20, help="Maximum number of samples to test")
    parser.add_argument("--attack-ratio", type=float, default=0.3, help="Ratio of attack samples (0.0-1.0)")
    ...
    parser.add_argument("--full-benchmark", action="store_true", help="Run full benchmark with 1000 samples")

    args = parser.parse_args()

    # Handle quick test and full benchmark flags
    if args.quick_test:
        args.max_samples = 20
        args.attack_ratio = 0.3
    elif args.full_benchmark:
        args.max_samples = 1000
        args.attack_ratio = 0.35

    print("NSL-KDD Intrusion Detection Benchmark with Google Gemini")
    print("=" * 60)
    ...
    try:
        ...
        benchmark = NSLKDDBenchmarkWithGemini(
